...
build('windows/playonline-run.c', probes/'playonline-run.exe', ['-mconsole'])
build('windows/startup-trace.c', probes/'startup-trace.dll', ['-shared', '-Wl,--kill-at', '-msse2'])
...
build('windows/graphics-check.c', probes/'graphics-check.exe', ['-ld3d8','-mconsole'])
if args.tests:
    ...
    build('tests/windows/game-window.c', tests/'game-window.exe', ['-mconsole'])
