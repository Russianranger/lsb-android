...
Current development: **the matching Box64 comparison is complete.** Both previously corrupted screens render correctly on 0.5.16 under Box64 at about 29 FPS. All five states captured under FEX also occur unchanged in working Box64, so the retained render states and line-pattern warning do not identify the fault. A CI-only fixture is testing the compressed textures and blending combinations found in the game captures. See [the comparison and investigation](docs/box64-fex-comparison-0516.md) and [handoff](docs/HANDOFF.md).

The requested arithmetic, DXVK-version and both actual-game captures are complete. No repeated device test or new APK is requested at this checkpoint. Keep Box64 as the working baseline, with prepared client `30251204_1`, original xiloader and working Termux server intact.
