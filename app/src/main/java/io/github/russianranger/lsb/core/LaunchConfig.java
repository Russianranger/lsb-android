...
public final class LaunchConfig {
    public final String host, region, polCore;
    public LaunchConfig(String host, String region) {
        this(host, region, "");
    }
    public LaunchConfig(String host, String region, String polCore) {
        ...
        this.host = host; this.region = region; this.polCore = polCore == null ? "" : polCore;
    }
    ...
    public String properties() throws IOException {
        Properties p = new Properties(); p.setProperty("schema", "1"); p.setProperty("host", host); p.setProperty("region", region); p.setProperty("polCore", polCore);
        StringWriter out = new StringWriter(); p.store(out, null); return out.toString();
    }
    public static LaunchConfig load(File f) throws IOException {
        if (!f.exists()) return new LaunchConfig("127.0.0.1", "US");
        Properties p = new Properties(); try (Reader r = new StringReader(FilesEx.read(f, 32768))) { p.load(r); }
        return new LaunchConfig(p.getProperty("host", "127.0.0.1"), p.getProperty("region", "US"), p.getProperty("polCore", ""));
    }
}
