...
Current development: **0.5.16 actual-game capture audited; FEX rendering remains unresolved.** The observer recorded 65,536 UP draws and 224 sampled vertices with no flagged sample values, then detached at frame 216. This does not clear all geometry or the later corrupted character-selection scene. Frame uploads succeed. See [the current audit and next comparison](docs/thor-game-capture-0516.md), [handoff](docs/HANDOFF.md), and [0.5.16 qualification](docs/d3d8-game-capture-0516.md).

The next device step uses the existing 0.5.16 APK: turn FEX off, retain DXVK 2.7.1 and the existing graphics settings, enable **Capture FFXI startup and graphics**, and capture the same menu under the previously working Box64 stack. Screenshot, Stop/export and disable capture afterward. The standalone arithmetic checks, actual-game DXVK comparison and FEX capture are complete. Preserve prepared client `30251204_1`, the original xiloader and working Termux server; no new runtime, client preparation or prefix reset is needed.
