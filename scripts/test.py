...
subprocess.run(['java', '-m', 'jdk.compiler/com.sun.tools.javac.Main', '--release', '8', '-d', str(classes), *map(str, sources), str(root/'tests/CoreTest.java'), str(root/'tests/PreparedClientTest.java'), str(root/'tests/LoginRequestTest.java')], check=True)
