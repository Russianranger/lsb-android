...
JNIEXPORT jint JNICALL Java_io_github_russianranger_lsb_NativePresentation_frame(JNIEnv *env,jclass cls,jobject surface,jint fd,jobject storage,jlongArray measures){
    ...
    if(!lsb_frame_valid(header)||(*env)->GetArrayLength(env,measures)<9)return -2;
    ...
    if(!pixels||capacity<LSB_MAX_PIXELS*4)return -3;
    ...
    if(!error){
        ...
        else for(uint32_t y=0;y<header[2];y++)lsb_bgra_rgba((uint32_t *)buffer.bits+(size_t)y*buffer.stride,(const uint32_t *)pixels+(size_t)y*header[1],header[1]);
