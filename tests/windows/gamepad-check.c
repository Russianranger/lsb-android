...
static BOOL centered(LONG value){return value>=32000&&value<=33500;}
...
int wmain(void){
    ...
    for(int step=0;step<4;step++){
        ...
        if(!ok){printf("FAIL: gamepad stage %d hr=%08lx button0=%u x=%ld y=%ld z=%ld rx=%ld ry=%ld rz=%ld hat=%lu\n",step,(unsigned long)last_hr,last.rgbButtons[0],last.lX,last.lY,last.lZ,last.lRx,last.lRy,last.lRz,last.rgdwPOV[0]);return 10+step;}
        printf("PASS: gamepad stage %d button0=%u x=%ld y=%ld z=%ld rx=%ld ry=%ld rz=%ld hat=%lu\n",step,last.rgbButtons[0],last.lX,last.lY,last.lZ,last.lRx,last.lRy,last.lRz,last.rgdwPOV[0]);
    }
