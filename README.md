...
Current development: **the matching Box64 comparison is complete.** Both previously corrupted screens render correctly on 0.5.16 under Box64 at about 29 FPS. All five states captured under FEX also occur unchanged in working Box64, so the retained render states and line-pattern warning do not identify the fault. A CI-only fixture now passes the synthetic compressed-texture and blending checks under both runtime stacks; the actual FEX game-rendering fault remains unresolved. See [the comparison and investigation](docs/box64-fex-comparison-0516.md) and [handoff](docs/HANDOFF.md).

The requested arithmetic, DXVK-version and both actual-game captures are complete. The next source inspection needs `FINAL FANTASY XI/FFXiMain.dll` from the same client; the support export contains its hash only. No repeated device test or new APK is requested at this checkpoint. Keep Box64 as the working baseline, with prepared client `30251204_1`, original xiloader and working Termux server intact.
