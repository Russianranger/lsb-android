...
public final class RuntimeService extends Service {
    ...
    @Override public int onStartCommand(Intent intent,int flags,int id){
        ...
        startForeground(2,new Notification.Builder(this,"runtime").setSmallIcon(android.R.drawable.ic_media_play).setContentTitle("LSB Windows checks").setContentText("Open the display or stop the runtime").setContentIntent(view).addAction(new Notification.Action.Builder(null,"Stop",stop).build()).setOngoing(true).build());
        ...
        final String renderer=intent.getStringExtra("renderer");final boolean sound=intent.getBooleanExtra("audio",true);
        ...
        worker=new Thread(()->{
            try{ClientRuntime.get(this).run(renderer,sound);}
