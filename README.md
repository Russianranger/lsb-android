...
Current development: **a concrete FEX CPU-feature mismatch is corrected and reproduced by ARM64 regression checks.** Exact-client inspection shows the Windows 3DNow report can prevent FFXI's math dispatcher from reaching SSE2. The corrected runtime makes Windows and CPUID agree; both x87 modes passed the before/after checks. APK 0.5.17 and immutable runtime v2 are undergoing full compatibility qualification. Actual phone rendering remains unconfirmed. See [the investigation](docs/ffxi-cpu-dispatch-0517.md) and [handoff](docs/HANDOFF.md).

The matching Box64 comparison is complete and renders both screens at about 29 FPS. The requested client DLLs were received and verified. Preserve prepared client `30251204_1`, original xiloader, Box64 fallback and the working Termux server. No repeated arithmetic, DXVK-version or diagnostic-capture comparison is needed.
