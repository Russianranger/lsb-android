...
with tempfile.TemporaryDirectory(prefix='lsb-wire-') as temporary:
    ...
    elif args.backend=='docker':command=['docker','run','--rm','--network','none','-v',str(folder)+':/session','-v',str(root/'tests/runtime')+':/tests:ro','-v',str(root/'out/presentation')+':/presentation:ro','lsb-runtime:test','python3','/tests/display_fixture.py','/session']
    else:
