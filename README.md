...
Current development: **0.5.16 bounded D3D8 game diagnostics**, qualified by all six applicable PR CI jobs. The Thor has the same missing menus and corruption under both DXVK 2.5.3 and 2.7.1, despite passing the standalone pixel checks. The optional capture observes the game's render states and sampled submitted positions. It is a diagnostic build, not a claimed rendering fix. See [the audit, qualification and device procedure](docs/d3d8-game-capture-0516.md), [handoff](docs/HANDOFF.md) and [validation](docs/validation.md).

Keep the prepared client `30251204_1`, original xiloader and working Termux server. The completed arithmetic and DXVK-version comparisons do not need repeating. Install 0.5.16 over the app and use one short launch with **Capture FFXI startup and graphics** enabled, DXVK 2.7.1 restored and FEX faster x87 on. Stop/export at the broken menu and disable capture afterward. No client re-import, re-preparation or runtime download is needed. Box64 remains the previously working rendering baseline.
