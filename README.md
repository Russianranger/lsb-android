...
Current development: **0.5.15 device results received**. Both FEX arithmetic modes pass all 128 graphics pixel samples on the Thor with DXVK 2.7.1, but the actual FFXI menu still renders incorrectly at about 13 FPS. The new DXVK warning identifies an unsupported line-pattern setting; no rendering fix is established. See [the three-export audit and next comparison](docs/thor-pixel-results-0515.md), [handoff](docs/HANDOFF.md) and [validation record](docs/validation.md).

The next comparison uses the existing APK: keep FEX faster x87 on, turn off **DXVK 2.7.1 · experimental** to select bundled 2.5.3, and launch once to the same menu. Screenshot, Stop/export, then restore 2.7.1. Keep Turnip 26, 60 Hz, Native Surface and shared-memory upload fixed. Preserve the prepared client `30251204_1`, original xiloader and working Termux server. Turning FEX off selects the preserved Box64 environment. Server migration remains a separate step; see [the server workflow](docs/milestone-050.md).
