...
subprocess.run(['java', '-m', 'jdk.compiler/com.sun.tools.javac.Main', '--release', '8', '-d', str(classes), *map(str, sources), str(root/'tests/CoreTest.java'), str(root/'tests/PreparedClientTest.java'), str(root/'tests/LoginRequestTest.java'), str(root/'tests/ServerAccountRequestTest.java'), str(root/'tests/SessionArchiveTest.java'), str(root/'tests/SessionTransactionTest.java')], check=True)
...
subprocess.run(['java', '-ea', '-cp', str(classes), 'SessionArchiveTest'], check=True)
subprocess.run(['java', '-ea', '-cp', str(classes), 'SessionTransactionTest'], check=True)
